...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    sudo cp `python -c 'import sys; print (sys.path[-1])'`/lalsuite.dylibs/lib*dylib ${HOME}/lib
    sudo cp `python -c 'import sys; print (sys.path[-1])'`/lalsuite.dylibs/liblal.*.dylib ${HOME}/lib/liblal.dylib
